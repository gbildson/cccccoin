...
{
    return GetBoolArg("-testnet", false) ? 40444 : 20444;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Cccccoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Cccccoin server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "getminingmode",          &getminingmode,          true,      false,      false },
    { "setminingmode",          &setminingmode,          true,      false,      true },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: cccccoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads()
{
        ...
        || weakPassword(mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use cccccoind";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a secure rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Cccccoin Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    //if (strMethod == "setminingmode"          && n > 0) ConvertTo<boost::int64_t>(params[0]);
